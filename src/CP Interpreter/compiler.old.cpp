...
void Compiler::visit(ASTEnumNode* astnode) {
	...
	for (size_t i = 0; i < astnode->identifiers.size(); ++i) {
		...
		auto var_id = scopes[nmspace].back()->declare_variable(astnode->identifiers[i]);
...
void Compiler::visit(ASTDeclarationNode* astnode) {
	...
	auto var_id = scopes[nmspace].back()->declare_variable(astnode->identifier);
...
void Compiler::visit(ASTAssignmentNode* astnode) {
	...
	auto var_id = astscope->find_declared_variable(astnode->identifier);
